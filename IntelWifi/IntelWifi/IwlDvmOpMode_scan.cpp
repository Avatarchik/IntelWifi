...
static void iwl_process_scan_complete(struct iwl_priv *priv)
{
    ...
    if (priv->scan_type != IWL_SCAN_NORMAL && !aborted) {
        ...
        err = iwl_scan_initiate(priv, priv->scan_vif, IWL_SCAN_NORMAL, priv->scan_request->channels[0]->band);
        if (err) {
            IWL_DEBUG_SCAN(priv, "failed to initiate pending scan: %d\n", err);
            aborted = true;
            goto out_complete;
        }
...
static void iwl_rx_scan_start_notif(struct iwl_priv *priv, struct iwl_rx_cmd_buffer *rxb)
{
    ...
    IWL_DEBUG_SCAN(priv, "Scan start: %d [802.11%s] (TSF: 0x%08X:%08X) - %d (beacon timer %u)\n",
                   notif->channel,
                   notif->band ? "bg" : "a",
...
static void iwl_rx_scan_complete_notif(struct iwl_priv *priv, struct iwl_rx_cmd_buffer *rxb)
{
    ...
    for (int i = 0; i < priv->num_stations; ++i) {
        IWL_DEBUG_SCAN(priv, "Station: " MAC_FMT "\n", MAC_BYTES(priv->stations[i].sta.sta.addr));
    }
    ...
    && priv->bt_status != scan_notif->bt_status) {
        if (scan_notif->bt_status) {
            ...
            /* BT off */
            priv->bt_traffic_load = IWL_BT_COEX_TRAFFIC_LOAD_NONE;
        }
        priv->bt_status = scan_notif->bt_status;
        // TODO: Implement
...
// line 368
static u16 iwl_get_active_dwell_time(struct iwl_priv *priv, enum nl80211_band band, u8 n_probes)
{
    if (band == NL80211_BAND_5GHZ)
        return IWL_ACTIVE_DWELL_TIME_52 + IWL_ACTIVE_DWELL_FACTOR_52GHZ * (n_probes + 1);
    else
        return IWL_ACTIVE_DWELL_TIME_24 + IWL_ACTIVE_DWELL_FACTOR_24GHZ * (n_probes + 1);
}
 ...
 */
static u8 iwl_get_single_channel_number(struct iwl_priv *priv, enum nl80211_band band)
{
    ...
    for (i = 0; i < sband->n_channels; i++) {
        ...
        for_each_context(priv, ctx) {
            busy = sband->channels[i].hw_value == le16_to_cpu(ctx->staging.channel);
            if (busy)
                break;
        }
...
// line 475
static int iwl_get_channel_for_reset_scan(struct iwl_priv *priv, struct ieee80211_vif *vif, enum nl80211_band band,
                                     ...
                                     struct iwl_scan_channel *scan_ch)
{
    ...
    for (i = 0, added = 0; i < priv->scan_request->n_channels; i++) {
        ...
        IWL_DEBUG_SCAN(priv, "Scanning ch=%d prob=0x%X [%s %d]\n",
                       ...
                       (scan_ch->type & SCAN_CHANNEL_TYPE_ACTIVE) ? "ACTIVE" : "PASSIVE",
                       (scan_ch->type & SCAN_CHANNEL_TYPE_ACTIVE) ? active_dwell : passive_dwell);
...
static u16 iwl_fill_probe_req(struct ieee80211_mgmt *frame, const u8 *ta,
                              const u8 *ies, size_t ie_len, const u8 *ssid,
                              u8 ssid_len, int left)
{
    ...
    /* ...next IE... */
    pos = &frame->u.probe_req_variable[0];
...
static int iwlagn_request_scan(struct iwl_priv *priv, struct ieee80211_vif *vif)
{
    ...
    if (!priv->scan_cmd) {
        priv->scan_cmd = (struct iwl_scan_cmd *)iwh_malloc(scan_cmd_size);
        if (!priv->scan_cmd) {
    ...
    if (iwl_is_any_associated(priv)) {
        ...
        switch (priv->scan_type) {
            ...
            case IWL_SCAN_NORMAL:
                interval = 100;//vif->bss_conf.beacon_int;
                break;
        }
        ...
        scan_suspend_time = (extra | ((suspend_time % interval) * 1024));
    ...
    switch (priv->scan_band) {
        case NL80211_BAND_2GHZ:
            ...
            chan_mod = le32_to_cpu(priv->contexts[IWL_RXON_CTX_BSS].active.flags & RXON_FLG_CHANNEL_MODE_MSK)
    ...
    if (priv->new_scan_threshold_behaviour)
        scan->good_CRC_th = is_active ? IWL_GOOD_CRC_TH_DEFAULT : IWL_GOOD_CRC_TH_DISABLED;
    else
        scan->good_CRC_th = is_active ? IWL_GOOD_CRC_TH_DEFAULT : IWL_GOOD_CRC_TH_NEVER;

    band = priv->scan_band;

    if (band == NL80211_BAND_2GHZ && priv->lib->bt_params && priv->lib->bt_params->advanced_bt_coexist) {
        /* transmit 2.4 GHz probes only on first antenna */
        scan_tx_antennas = first_antenna(scan_tx_antennas);
    }

    priv->scan_tx_ant[band] = iwl_toggle_tx_ant(priv, priv->scan_tx_ant[band], scan_tx_antennas);
     ...
     */
    if (test_bit(STATUS_POWER_PMI, &priv->status) && !(priv->hw->conf.flags & IEEE80211_CONF_IDLE)) {
        /* rx_ant has been set to all valid chains previously */
        active_chains = rx_ant & ((u8)(priv->chain_noise_data.active_chains));
        if (!active_chains)
    ...
    if (priv->lib->bt_params && priv->lib->bt_params->advanced_bt_coexist && priv->bt_full_concurrent) {
    ...
    rx_chain |= priv->nvm_data->valid_rx_ant << RXON_RX_CHAIN_VALID_POS;
    ...
    switch (priv->scan_type) {
        case IWL_SCAN_NORMAL:
            cmd_len = iwl_fill_probe_req(
                                         ...
                                         priv->nvm_data->hw_addr, //vif->addr,
...
//            BUG();
    ...
    scan->filter_flags |= (RXON_FILTER_ACCEPT_GRP_MSK | RXON_FILTER_BCON_AWARE_MSK);
    ...
    cmd.len[0] += le16_to_cpu(scan->tx_cmd.len) + scan->channel_count * sizeof(struct iwl_scan_channel);
    cmd.data[0] = scan;
    ...
    if (ret) {
        DebugLog("iwlagn_set_pan_params FAILED");
        clear_bit(STATUS_SCAN_HW, &priv->status);
        return ret;
    }
                      ...
                      enum nl80211_band band)
{
    ...
    if (test_bit(STATUS_SCAN_HW, &priv->status)) {
        IWL_DEBUG_SCAN(priv, "Multiple concurrent scan requests in parallel.\n");
        return -EBUSY;
    }
    ...
    IWL_DEBUG_SCAN(priv, "Starting %sscan...\n", scan_type == IWL_SCAN_NORMAL ? "" : "internal short ");
    ...
    if (ret) {
        IWL_DEBUG_SCAN(priv, "SCAN FAILED");
        clear_bit(STATUS_SCANNING, &priv->status);
        priv->scan_type = IWL_SCAN_NORMAL;
        return ret;
    }
